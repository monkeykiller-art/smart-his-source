...
describe('ProtectedRoute', () => {
  ...
  it('renders content for an authenticated session', () => { const session = { accessToken: 'a', refreshToken: 'r', userId: 1, username: 'doctor', realName: '张医生', deptId: 2, deptName: '内科', roles: ['DOCTOR'] }; useAuthStore.setState({ session }); render(<MemoryRouter initialEntries={['/patient']}><Routes><Route element={<ProtectedRoute />}><Route path="/patient" element={<div>患者页</div>} /></Route></Routes></MemoryRouter>); expect(screen.getByText('患者页')).toBeInTheDocument() })
})

...
type TriageValues = { patientId: string; triageLevel: number; chiefComplaint: string; vitalSigns?: string }
...
export function InpatientEmergencyPage() {
  ...
  return <>
    ...
    <Modal title="新增急诊分诊" open={triageOpen} onCancel={() => setTriageOpen(false)} onOk={() => triageForm.submit()} confirmLoading={triage.isPending}><Form form={triageForm} layout="vertical" onFinish={values => triage.mutate(values)}><Form.Item name="patientId" label="患者编号" rules={[{ required: true, message: '请输入患者编号' }, { pattern: /^[1-9]\d*$/, message: '请输入有效的数字患者编号' }]}><Input inputMode="numeric" /></Form.Item><Form.Item name="triageLevel" label="分诊级别" rules={[{ required: true }]}><Select options={[1, 2, 3, 4].map(value => ({ value, label: triageMeta[value as keyof typeof triageMeta][0] }))} /></Form.Item><Form.Item name="chiefComplaint" label="主诉" rules={[{ required: true }]}><Input.TextArea /></Form.Item><Form.Item name="vitalSigns" label="生命体征"><Input placeholder="体温、脉搏、血压、血氧" /></Form.Item></Form></Modal>
